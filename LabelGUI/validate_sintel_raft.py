...
def pad_to_multiple_of_8(x: torch.Tensor):
    """
    RAFT expects H and W divisible by 8.
...
def compute_raft_flow(img_a_bgr, img_b_bgr, model, transforms, device):
    """
    Returns H x W x 2 optical flow in pixel units.
    """
    img_a_rgb = cv2.cvtColor(img_a_bgr, cv2.COLOR_BGR2RGB)
    img_b_rgb = cv2.cvtColor(img_b_bgr, cv2.COLOR_BGR2RGB)

    img_a = torch.from_numpy(img_a_rgb).permute(2, 0, 1).unsqueeze(0)
    img_b = torch.from_numpy(img_b_rgb).permute(2, 0, 1).unsqueeze(0)

    img_a, img_b = transforms(img_a, img_b)

    img_a, orig_h, orig_w = pad_to_multiple_of_8(img_a)
    img_b, _, _ = pad_to_multiple_of_8(img_b)

    img_a = img_a.to(device)
    img_b = img_b.to(device)

    with torch.inference_mode():
        flow_predictions = model(img_a, img_b)

    # Torchvision RAFT returns a list of iterative flow predictions.
    # The last one is the refined/final prediction.
    flow = flow_predictions[-1][0]
...
def save_visual_example(img_a, img_b, pred_flow, gt_flow, epe_map, valid_mask, out_path: Path):
    ...
    panels = [
        ...
        add_title(pred_vis, "RAFT Estimated Flow"),
...
def load_raft_model(model_size: str, device: torch.device):
    if model_size == "small":
...
def main():
    ...
    parser.add_argument("--model-size", default="small", choices=["small", "large"])
    ...
    if args.run_name:
        ...
        run_name = f"raft_{args.model_size}_{args.pass_name}_v1"
    ...
    print("Loading RAFT model. The first run may download pretrained weights...")

    model, transforms, weights = load_raft_model(args.model_size, device)
    ...
    metrics = {
        "method": f"Torchvision RAFT {args.model_size}",
        "torchvision_model": f"raft_{args.model_size}",
        "weights": str(weights),
